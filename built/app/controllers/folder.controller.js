...
exports.delete = (req, res) => {
    ...
    folderProvider.delete(folderId, userId, role)
        ...
        if (!data) {
            ...
            examProvider.updateOne({ "folderId": folderId, userId: userId }, { folderId: null })
                .then(() => {
                res.send({
...
exports.update = (req, res) => {
    ...
    folderProvider.update(folderId, userId, req.body.folder)
        .then(data => {
        if (!data) {
            res.status(404).send({
            ...
            res.send({ message: "Exam was updated successfully." });
    })
        .catch(err => {
        res.status(500).send({
