...
describe("Login controller Integration tests", () => {
    ...
    describe("Root: Delete a user", () => {
        ...
        it("Login with an existing user and verified", (done) => {
            const query = querystring.stringify({
                username: usersData.root.username,
                password: existingPassword
            });
            request(app).get(`/login?${query}`)
                .end(function (err, res) {
    ...
    describe("Admin: Delete a user", () => {
        ...
        it("Login with an existing user and verified", (done) => {
            const query = querystring.stringify({
                username: usersData.verified.username,
                password: existingPassword
            });
            request(app).get(`/login?${query}`)
                .end(function (err, res) {
    ...
    describe("Forgot password with an existing user", () => {
        it("Existing user recovers password", (done) => {
            const username = usersData.toUpdate.username;
            request(app)
        ...
        it("Submit new password", (done) => {
            db.user
                .findOne({username: usersData.toUpdate.username})
                .then(user => {
                    if (!user) throw new Error("User doesnt exist");
                    db.resetPasswordToken.findOne({userId: user.id})
                        .then(resetToken => {
                            if (!resetToken) throw new Error("Token doesnt exist");
                            request(app)
        ...
        it("Login with a new password", (done) => {
            const query = querystring.stringify({
                username: usersData.toUpdate.username,
